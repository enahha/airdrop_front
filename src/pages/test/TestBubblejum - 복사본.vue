...
import { createTree, mintToCollectionV1, parseLeafFromMintToCollectionV1Transaction, findLeafAssetIdPda, getAssetWithProof, verifyCollection, updateMetadata, mintV1, mplBubblegum, getCurrentRoot, fetchMerkleTree } from '@metaplex-foundation/mpl-bubblegum'
...
export default defineComponent({
  ...
  data() {
    return {
      solanaAddress: '',
      bubbleGumTreeAddress: '6BLMNxey3gcPacV8ReWLsZbipZ7guT7qHeddXDRFPoDL',
      ...
      nftData: {
        ...
        collectionMint: "GVD9AYbivmKkNFrRvHRNWmpTMCuRLorRj3eScfUwyfcJ",
      },
      updateData: {
        assetId: '',
        name: '',
  ...
  methods: {
    ...
    async mintSolanaNftBubblegum() {
      try {
        ...
        console.log('CNFT Asset Index >>>>> ', leaf.nonce);
        console.log('CNFT Asset Owner >>>>> ', leaf.owner);
    ...
    async mintSolanaNftBubblegumMintV1() {
      try {
        ...
        const assetWithProof = await getAssetWithProof(umi, assetId[0], { das: DAS_URL });
    ...
    async parseLeafWithRetries(umi, signature, retries = 5, delay = 2000) {
