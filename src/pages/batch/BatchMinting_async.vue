<template>
        ...
        <div class="row justify-center text-white q-pt-lg">
          <table>
              ...
              <td>
                <q-btn @click="confirmMinting" :loading="loading" class="btn-purple" no-caps style="padding: 0px 10px; height: 80px; width: 200px;" tabindex="2">
                  ...
                  <template v-slot:loading>
                    <q-spinner-hourglass class="on-left" size="1.5em" />
                    Processing...
                  </template>
                </q-btn>
              </td>
            ...
            <tr>
              ...
              <td><h2>{{ totalCompletedCount }}&nbsp;&nbsp;/&nbsp;&nbsp;5000</h2></td>
            </tr>
            <tr>
              <td></td>
            </tr>
            ...
            <table border="0" width="100%" style="margin: 0px 0px 0px 0px; max-width: 1200px;" cellpadding="0" cellspacing="0" class="text-white">
              ...
              <tbody>
                <tr v-for="(item, index) in airdropLogList" :key="index">
                  ...
                  <td>{{ item.new_nft_id && item.new_mint_account_key ? item.new_nft_id : item.resultNftId }}</td>
                  <td>{{ item.reg_time }}</td>
                </tr>
              </tbody>
            </table>
  ...
  <q-dialog v-model="confirmMint">
    <q-card class="bg-white" style="width: 220px;">
      <q-card-section class="row items-center" style="">
        ...
        <span class="q-ml-sm text-black">Start airdrop?</span>
      </q-card-section>
      <q-separator />
  ...
  <!-- 에어드랍 처리 완료 모달 -->
  <q-dialog v-model="processCompletedModal">
    <q-card class="bg-white" style="width: 220px;">
      <q-card-section class="row items-center" style="">
        <q-icon name="info" color="blue" size="md" />
        <!-- <span class="q-ml-sm text-black">{{ orderQuantity }} Punky Kongz?</span> -->
        <span class="q-ml-sm text-black">Process completed!</span>
      </q-card-section>
      <q-separator />
      <q-card-actions align="around">
        <q-btn style="width: 100%;" label="OK" color="blue" v-close-popup />
      </q-card-actions>
    </q-card>
  </q-dialog>
...
export default defineComponent({
  ...
  methods: {
    ...
    async batchMintingNew() {
      ...
      const concurrencyLimit = 5 // 동시 실행 개수
      const queue = [...this.airdropLogList.filter(item => item.statusLabel === 'WAIT' || item.statusLabel === 'FAILED')]

      const executeBatch = async (batch) => {
        await Promise.all(
          batch.map(async (airdropLogObj) => {
            try {
              console.log('Processing:', airdropLogObj.seq)
              const result = await this.doMinting(airdropLogObj)
              ...
              if (result && index !== -1) {
                ...
                this.airdropLogList[index].resultNftId = result

                // 총 처리 완료 갯수 + 1
                this.totalCompletedCount++
                // this.totalCompletedCount 가 5000인 경우, 처리중 -> 처리완료
                // console.log('this.totalCompletedCount: ' + this.totalCompletedCount)
                if (this.totalCompletedCount == this.$PUNKYKONGZ_TOTAL_SUPPLY) {
                  this.loading = false // 처리중 표시 종료
                  this.processCompletedModal = true // 처리 완료 메시지 표시
                }
              } else if (index !== -1) {
      ...
      while (queue.length > 0) {
        const batch = queue.splice(0, concurrencyLimit) // 배치 단위로 가져오기
        await executeBatch(batch) // 배치 실행
      }

      console.log('Batch Minting Completed')
      this.refresher() // UI 갱신
      this.selectedAll = false
    },
    async batchMinting() {
      alert('not this!!!')
    ...
    async doMinting(airdropLogObj) {
      ...
      const paramNftInfo = {
        projectSeq: this.projectSeq,
        nftId: airdropLogObj.new_nft_id,
      }
      ...
      if (newMintingLogVo == null) {
        ...
        if (nftInfoVo != null) {
          ...
          resultUpdateStatus = await this.$axios.post('/api/nftinfo/updateNftInfoStatus', paramNftInfoStatusFailed)
          console.log("updateNftInfoStatus  >>", resultUpdateStatus.data)
          // console.log(e)
        }
        // airdrop_log 테이블에 해당 data의 status를 실패(5)로 수정
        if (nftInfoVo != null) {
          ...
          resultUpdateStatus = await this.$axios.post('/api/airdropLog/updateAirdropLogStatus', paramAirdropLogStatusFailed)
          console.log("updateAirdropLogStatus  >>", resultUpdateStatus.data)
          // console.log(e)
        }
        return
      }
      ...
      .add(
        mintToCollectionV1(umi, {
          ...
          metadata: {
            ...
            symbol,
            uri,
            sellerFeeBasisPoints: this.sellerFeeBasisPoint * 100,
            collection: {
              key: collectionMint,
              verified: false,
            },
