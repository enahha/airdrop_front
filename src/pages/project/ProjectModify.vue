<template>
  <q-page class="page-default q-pa-md page-1200 q-page-mini">
    ...
    <div class="row justify-center q-pb-md">
      <div class="col-12">
        <q-input v-model="adminWalletAddress" ref="refAdminWalletAddress" :rules="[required, val => minLength(val, 1), val => maxLength(val, 200)]" clearable standout tabindex="1" />
      </div>
    </div>
    <div class="row justify-center q-pt-lg">
      <div class="col-12">
    ...
    <div class="row justify-center q-pb-md">
      <div class="col-12">
        <q-input v-model="listingWalletAddress" ref="refListingWalletAddress" :rules="[required, val => minLength(val, 1), val => maxLength(val, 200)]" clearable standout tabindex="1" />
      </div>
    </div>
    <div class="row justify-center q-pt-lg">
      <div class="col-12">
    ...
    <div class="row justify-center q-pb-md">
      <div class="col-12">
        <q-input v-model="feeWalletAddress" ref="refFeeWalletAddress" :rules="[required, val => minLength(val, 1), val => maxLength(val, 200)]" clearable standout tabindex="1" />
      </div>
    </div>
    <div class="row justify-center q-pt-lg">
      <div class="col-12">
    ...
    <div class="row justify-center q-pb-md">
      <div class="col-12">
        <q-input v-model="listingWalletSecretKey" ref="refListingWalletSecretKey" :rules="[required, val => minLength(val, 1), val => maxLength(val, 200)]" clearable standout tabindex="1" />
      </div>
    </div>
    <div class="row justify-center q-pt-lg">
      <div class="col-12">
    ...
    <div class="row justify-center q-pb-md">
      <div class="col-12">
        <q-input v-model="feeWalletSecretKey" ref="refFeeWalletSecretKey" :rules="[required, val => minLength(val, 1), val => maxLength(val, 200)]" clearable standout tabindex="1" />
      </div>
    </div>
    <div class="row justify-center q-pt-lg">
      <div class="col-12">
      ...
      <div class="col-12">
        <q-card class="my-card">
          <q-item>
            ...
            <q-item-section>
              <div class="row">
                <q-item-label class="col-12 text-weight-bold">{{ projectName }} {{ projectSymbol }}</q-item-label>
              </div>
            </q-item-section>
          </q-item>
        </q-card>
    ...
    <div class="row justify-center q-pb-md">
      <div class="col-12">
        <div class="" style="max-width: 250px">
          <q-input v-model="mintStartTime" ref="refMintStartTime" :rules="[required, val => minLength(val, 16), val => maxLength(val, 50)]" outlined tabindex="7">
    ...
    <div class="row justify-center">
      <div class="col-12">
        <div class="" style="max-width: 250px">
          <q-input v-model="mintEndTime" ref="refMintEndTime" :rules="[required, val => minLength(val, 16), val => maxLength(val, 50)]" outlined tabindex="8">
            <template v-slot:prepend>
              <q-icon name="event" class="cursor-pointer">
                <q-popup-proxy cover transition-show="scale" transition-hide="scale">
    ...
    <div class="row justify-center q-pt-lg btn-wrap">
      ...
      <div class="col-6 text-right">
        <q-btn class="btn" color="primary" text-color="black" size="lg" style="width: 98%;" @click="modifyProject" tabindex="3">
          {{ $t('modify') }}
        </q-btn>
      </div>
    </div>
...
export default defineComponent({
  ...
  created: function () {
    // 키 설정
    this.seq = this.$route.query.s
    // NtfProject 조회
    this.selectNtfProject()
  },
  mounted: function () {},
  methods: {
    async selectNtfProject() {
      const param = {
        seq: this.seq,
      }
      const result = await this.$axios.get('/api/project/selectProject', { params: { ...param } })
      if (result.data) {
        console.log(result.data)
        this.seq = result.data.seq
        this.projectId = result.data.project_id
        this.projectSymbol = result.data.project_symbol
        this.projectName = result.data.project_name
        this.projectNameKo = result.data.project_name_ko
        this.contractAddress = result.data.contract_address
        this.jsonPath = result.data.json_path
        this.adminWalletAddress = result.data.admin_wallet_address
        this.listingWalletAddress = result.data.listing_wallet_address
        this.feeWalletAddress = result.data.fee_wallet_address
        this.listingWalletSecretKey = result.data.listing_wallet_secret_key
        this.feeWalletSecretKey = result.data.fee_wallet_secret_key
        this.projectImageUrl = result.data.image_url
        this.mainnet.value = result.data.mainnet
        this.description = result.data.description
        this.descriptionKo = result.data.description_ko
        this.creatorFee = result.data.creator_fee
        this.mintStartTime = result.data.mint_start_time
        this.mintEndTime = result.data.mint_end_time
        this.mintStartBlock = result.data.mint_start_block
        this.mintEndBlock = result.data.mint_end_block
      } else {
    ...
    modifyProject() {
      ...
      const param = {
        ...
        admin_wallet_secret_key : this.adminWalletSecretKey,
      ...
      console.log(param)
      this.$axios.post('/api/project/updateProject', param)
        .then((result) => {
    ...
    this.projectImageUrl = fileNameNew // 토큰 로고 URL 설정
    // this.$refs.uploaderObj.reset()
  },
  ///////////////////////////////////////////////////////////////////////////
  // validation
