<template>
  <div class="header" :class="{ 'is-open': isOpen }">
    ...
    <div>
      <h1 class="logo"><a @click="goHome('')"><img src="images/logo.png" alt=""></a></h1>

      <div class="inner">
        <nav class="pcNav">
          <ul>
            <li><a @click.prevent="goHome('')">MINT</a></li>
            <li><a @click.prevent="goHome('')">WTEC</a></li>
            <li><a @click.prevent="goHome('')">ALYA <span>l</span> FITRI</a></li>
            <li><a @click.prevent="goHome('')">BITCOIN</a></li>
            <li><a @click.prevent="goHome('')">MEMBERSHIP</a></li>
          </ul>
          ...
          <div class="right">
            ...
            <a @click.prevent="selectNation('')" class="mypage">My Page</a>
          </div>
        </nav>
  ...
  <div id="mypage">
    <div>
      <h2>My Page</h2>
      <ul class="tab">
        <li :class="{ on: activeTab === 'sec01' }" @click.prevent="setActiveTab('sec01')">Retained <br>status</li>
        <li :class="{ on: activeTab === 'sec02' }" @click.prevent="setActiveTab('sec02')">Withdrawal <br>request</li>
      ...
      <section v-show="activeTab === 'sec02'" class="sec02">
        <form action="">
          ...
          <div class="box1">
            <div>
              <p class="title">Withdrawal Quantity</p>
          ...
          <div class="box2">
            <p class="title">Withdrawal address</p>
          ...
          <div class="box4">
            <p class="title">Withdrawal Policy</p>
            <div class="formboard">
              <p>Withdrawal fee is 0.5% of the WTEC(or TH) quantity requested for withdrawal.</p>
              <p>You can withdraw at least 20 WTEC(or TH) within the amount of WTEC(or TH) you own.</p>
              <p>Withdrawal quantity input is limited to 1 digit after the decimal point.</p>
              <p>Once the withdrawal is processed, the amount requested minus the fee will be deposited into the wallet from which the withdrawal was made.</p>
              <p>Withdrawal applications are available 24 hours a day, and are processed sequentially according to the system after the withdrawal application.</p>
            </div>
          </div>
  ...
  <MintingApplyModal ref="refMintingApplyModal" @callback-applyMinting="callbackSolanaWalletAddress"/>
  <MemetoonAccountModal ref="refMemetoonAccountModal" @callback-memetoonAccount="callbackMemetoonAccount"/>
  ...
  methods: {
    ...
    async fetchData() {
      try {
        ...
        console.log(pendingRewardsList)
    ...
    showMintingApplyModal() {
      this.$refs.refMintingApplyModal.solanaWalletAddress = this.userVo.solana_wallet_address
      this.$refs.refMintingApplyModal.show()
    },
    showMemetoonIdRegisterModal() {
      this.$refs.refMemetoonAccountModal.memetoonAccount.id = this.userVo.memetoon_id
      this.$refs.refMemetoonAccountModal.memetoonAccount.pwd = this.userVo.memetoon_pwd
      this.$refs.refMemetoonAccountModal.show()
    },
    async callbackSolanaWalletAddress(sollanaWalletAddress) {
      ...
      this.$refs.refMintingApplyModal.close() // 모달창 닫기
    },
    async callbackMemetoonAccount(memetoonAccount) {
      ...
      this.$refs.refMemetoonAccountModal.close()
    },
  }
})
</script>
